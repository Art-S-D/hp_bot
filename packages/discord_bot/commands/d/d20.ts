...
async function reply_roll(
    ...
    return [response, value, value >= diff];
}
...
export async function d20(msg: Message, player: IPlayer | null, die: d20grammar): Promise<[Message, number, boolean]> {
...
export async function d(msg: Message, player: IPlayer | null) {
    ...
    const [response, _val, success] = await d20(msg, player, die);

    if (!success && isLearningRoll(die) && player != null)
