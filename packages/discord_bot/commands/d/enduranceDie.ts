...
export function isLearningRoll({ bonus1, bonus2, reroll }: Id20): boolean {
    return (bonus1 === "esprit" && bonus2 === "magie") || (bonus1 === "magie" && bonus2 === "esprit");
}

export async function addEnduranceRerollReactions(msg: Message, player: IPlayer, originalMessage: Message) {
    const reaction = await messageButton(msg, enduranceRollEmojis, [originalMessage.author.id]);
    ...
    if (diff) {
        originalMessage.reply(`jet d'endurance! \t (!d + endurance + magie / esprit | ${diff})`);
        ...
        if (diff == 20 && rollResult < 20) msg.reply("Oh non, vous être trop fatigué 😴");
    }
