...
function runResponses(msg: Message) {
    if (msg.content.toLowerCase().includes("nircosia")) msg.reply("son nom c'est nico");
    ...
    if (
        (msg.content.toLowerCase().includes("kate") || msg.content.toLowerCase().includes("kane")) &&
        msg.author.id !== client?.user?.id
    )
        msg.reply("attend, on parle de Kate ou de Kane ???");
    if (msg.content.toLowerCase().includes("jasper")) msg.react(emojiFromString(msg, "onatujasper"));
    if (msg.content.toLowerCase().includes("scenario") || msg.content.toLowerCase().includes("scénario"))
        msg.react(emojiFromString(msg, "allons_ruiner_le_scenario"));
}

async function runCommand(msg: Message) {
    if (msg.content[0] !== "!") return;

    const _command: string | undefined = Object.keys(commands).find((x) => msg.content.substring(1).startsWith(x));
...
async function login() {
    await client.login("NjY4NDI3NDg1MzY4NjgwNDUw.XiRH2Q.GCVmxJ9K8ash5GXox8TMX70zfEg").catch((e) => login());
}
login();
